...
export class ParticleEngine {
  ...
  private substep(dt: number, tiltX: number, tiltY: number): void {
    // Cloth's spring constraints (solveCloth, in physics.ts) only exist on
    // the CPU path — there's no GPU equivalent. Left alone, selecting Cloth
    // while WebGPU is the active backend spawns the pinned grid but never
    // solves the springs, so it just falls apart instead of draping. Force
    // compute back to CPU whenever springs exist; render() already knows how
    // to do GPU-accelerated rendering of CPU-stepped particles (that's the
    // `this.compute === "cpu"` branch inside `if (this.gpu)` below in
    // render()), so this only costs the cloth-specific physics, not the
    // WebGPU visuals. Reverts to GPU compute automatically once springs are
    // cleared (e.g. switching to another generator).
    if (this.gpu) {
      this.compute = this.springs.length > 0 ? "cpu" : "webgpu";
    }
    this.totalTime += dt;
