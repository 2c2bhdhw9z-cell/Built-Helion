...
fn integrate(@builtin(global_invocation_id) id: vec3<u32>) {
  ...
  // Grid / spatial hash logic for flocking, collisions, SPH fluid, and n-body
      ...
      for (var x: i32 = -1; x <= 1; x += 1) {
        ...
        if (cx >= 0 && cx < i32(params.gridCols) && cy >= 0 && cy < i32(params.gridRows)) {
          ...
          for (var idx: u32 = 0u; idx < count; idx += 1u) {
            ...
            if ((params.flags & 8u) != 0u) { // N-body gravity (local-neighborhood approx — mirrors the CPU n>900 path in physics.ts, not full long-range; see the coarse-grid note there)
               if (d2 > 0.0000001) {
                  let mass2 = lifeMassPhase[j].y;
                  let softened = d2 + params.softening * params.softening;
                  let g = (params.nbodyG * mass * mass2) / (softened * sqrt(softened));
                  acc -= d * g; // d = p - p2 (self - other), so -d points toward the neighbor
               }
            }
          }
        }
