...
describe("testWebhook delivery recording (Item 20)", () => {
  it("records a test delivery that appears in the owner's deliveries list", async () => {
    // Register a webhook to an unreachable URL — the outbound POST will fail, but
    // the delivery MUST still be recorded (ok=false) so it shows in the log.
    const hook = await tokens.insertWebhook("hook-owner", "http://127.0.0.1:9/hook");
  ...
  it("is owner-scoped: a user cannot test someone else's webhook", async () => {
    const hook = await tokens.insertWebhook("hook-alice", "http://127.0.0.1:9/hook");
