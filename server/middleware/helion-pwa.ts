/**
 * Deployed-app (Nitro) half of the Helion PWA chrome. Auto-registered as
 ...
 * - `/__helion/manifest.webmanifest` → per-app-named manifest (kept out of
 *   public/ so this dynamic response is the only one).
 * - Other HTML documents → stream-inject PWA + OG head tags at `</head>`.
 *   OG identity is baked via `virtual:helion-og-identity` at `vite build`
...
function requestHost(event: HelionPwaEvent): string {
  return (
...
export default async function helionPwaMiddleware(
  event: HelionPwaEvent,
